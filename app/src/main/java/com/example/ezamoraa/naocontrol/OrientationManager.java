    ...
    public void onSensorChanged(SensorEvent event){
        ...
        if (mGravity != null && mGeomagnetic != null){
            ...
            if (success){
                ...
                if (counter < averageSamples){
                    pitch += orientation[1];
                    yaw += orientation[2];
                }
                else if (counter == averageSamples ){
                    filteredPitch = Math.max(Math.min(pitch/averageSamples, maxPitch), minPitch);
                    filteredYaw = Math.max(Math.min(yaw/averageSamples, maxYaw), minYaw);
                    pitch = 0;
                    yaw = 0;
                    counter = -1;
                }
